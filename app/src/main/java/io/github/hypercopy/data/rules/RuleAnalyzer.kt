...
object RuleAnalyzer {
    ...
    private fun analyzeUrl(text: String): List<Suggestion> {
        ...
        if (isTakeoutShortLinkHost(host) && hasTakeoutPickupHint(text)) {
            ...
            val cabinetRegex = "([A-Za-z0-9]{0,3}号?柜外卖柜\\s*\\d+\\s*格口|[A-Za-z0-9]{0,3}号?柜\\s*\\d+(?:号)?\\s*格口|\\d+\\s*号?\\s*柜[^\\s，。]{0,5}\\d+\\s*格口|外卖柜\\s*\\d+\\s*格口|格口号?[:：]?\\s*\\d+|[A-Za-z0-9]{1,3}号?柜|\\d+(?:号)?\\s*格口|[A-Za-z0-9]{0,3}柜\\s*\\d+)"
